...
class Nexus extends EventEmitter<NexusEvents> {
    ...
    constructor(public readonly options: NexusConstructOptions) {
        ...
        this.ws = new WebSocket(this.options.password, this.options.host, this.options.wsport);
        this.ws.ondebug = (m) => this.emit("wsLog", m);

        this.rest = new RESTServer(this.options.password, this.options.host, this.options.restport);
